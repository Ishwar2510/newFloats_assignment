...
function Details() {
  ...
  async function fetchData(){
    ...
    const cResponse = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityname}&appid=${API_KEY}`)
      ...
      <div className="homeContainer">
        ...
        <div className = "foreCastContainer">
        {
          ...
          <div className = "indForeCast" id ="cdetails" >
                  <h4>{`${new Date().toLocaleDateString().split("/").join("-")} / ${new Date().toLocaleTimeString().split(":")[0]}:${new Date().toLocaleTimeString().split(":")[1]}` }</h4>
                  <img src= {`http://openweathermap.org/img/wn/${currentData.weather[0].icon}@2x.png`}></img>
                  <h4>{(currentData.main.temp - 273.15).toFixed(0)}<sup>o</sup>c </h4>
